...
for i1 = 1:zt 
  ...
  % gray scale opening
	O1 = ordfilt2(ordfilt2(ut_es2(M1, 3), 1, ones(5)), 25, ones(5));
	O2 = ordfilt2(ordfilt2(ut_es2(M2, 3), 1, ones(5)), 25, ones(5));
	% gray scale closing 
  ...
  O1 = ordfilt2(ut_es2(ut_undec2(ut_dec2(O1)), 3), 49, ones(7));
  O2 = ordfilt2(ut_es2(ut_undec2(ut_dec2(O2)), 3), 49, ones(7));
  % select valid image region 
  O1 = O1(4:z1+3,4:s1+3);
  O2 = O2(4:z1+3,4:s1+3);

 	% select coefficients and store them
  E(i1) = {ut_selc(M1-O1, M2-O2, ap)};
...
for i1 = zt:-1:1
  % dilate 
  M1 = ordfilt2(ut_es2(ut_undec2(M1), 3), 49, ones(7));
 	% select valid image region 
