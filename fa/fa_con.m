...
for i1 = 1:zt 
  ...
  % perform filtering 
  G1 = conv2(conv2(ut_es2(M1,2), w, 'valid'),w', 'valid');
  G2 = conv2(conv2(ut_es2(M2,2), w, 'valid'),w', 'valid');
 
  % decimate, undecimate and interpolate 
  M1T = conv2(conv2(ut_es2(ut_undec2(ut_dec2(G1)), 2), 2*w, 'valid'),2*w', 'valid');
  M2T = conv2(conv2(ut_es2(ut_undec2(ut_dec2(G2)), 2), 2*w, 'valid'),2*w', 'valid');
 
  % select coefficients and store them
  E(i1) = {ut_selc(M1./(M1T+eps)-1, M2./(M2T+eps)-1, ap)};
